defmodule Superlogica.Chat.Chat do
  ...
  defp listen_and_answer(contact, stage, "2") do
    Logger.info("#{__MODULE__}.listen_and_answer/1", ansi_color: :blue)
    #//VERIFICAR ESTÁGIO
    cond do
  ...
  defp listen_and_answer(contact, stage, "6") do
    ...
    cond do
        ...
        String.contains?(contact["message"], stage["info_responses"]) ->
          Logger.info(Messages.get_stage(35)["message"], ansi_color: :yellow) #//INFORMAÇÃO
  ...
  defp listen_and_answer(contact, stage, "11") do
    Logger.info("#{__MODULE__}.listen_and_answer/3", ansi_color: :blue)
  ...
  defp handle_area_message(value) do
    ...
    |> Enum.zip(value)
    |> Enum.reduce("", fn {number, %{"st_nome_are" => area}}, str ->
      "#{str}#{number} - #{area}\n"
    end)
  end
  ...
  defp area_by_option(choice, contact) do
    %{"id_area_are" => area_id, "st_nome_are" => area_name} = contact["common_areas"]
