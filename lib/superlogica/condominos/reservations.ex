defmodule Superlogica.Condominos.Reservations do
  ...
  @common_areas ["vl_valor_rec", "id_area_are", "st_nome_are", "id_condominio_cond"]
  ...
  defp get_fields(value, list \\ @common_areas) do
    Enum.reduce(list, %{}, fn e, map -> Map.put(map, e, Map.get(value, e)) end)
  end

  def find_area_by_id(condominium_id, area_id) do
    common_areas(condominium_id)
    |> Enum.filter(fn %{"id_area_are" => id} -> id == "#{area_id}" end)
    |> List.first()
  end
  ...
  def do_reservation(condominium_id, apartment_id, area_id, reservation_date) do
    %{"vl_valor_rec" => price} = find_area_by_id(condominium_id, area_id)
